...
def parse_args():
    ...
    parser.add_argument("--batch-size", type=int, default=32,
        help="batch-size")
    parser.add_argument("--epoch", type=int, default=600, # 50 for test standard is 600,
        help="epoch")
...
def reorder_shape_dim(shape):
    new_shape = np.zeros(2048)
    shape = shape
    j=0
    while j<4:
        offset = j*512
        i = 0
        while i<512:
            new_shape[i + offset] = shape[int(i/2) + offset]
            new_shape[i+1 + offset] = shape[int(i/2)+256 + offset]
            i+=2
        j+=1
    return new_shape
...
def reorder_shape(shape):
    all_shapes = []
    for line in shape:
        all_shapes.append(reorder_shape_dim(line))
    
    return torch.from_numpy(np.array(all_shapes))
...
if __name__ == '__main__':
    ...
    num_workers = 8 # alex changes should be equal to the number of cores (I only have 4 cores)
    ...
    for current_epoch in range(all_epoch):
        ...
        for idx, (train_x, train_label, train_label_weight, train_shape) in enumerate(train_loader):
            ...
            train_shape = reorder_shape(train_shape) # alex changes
